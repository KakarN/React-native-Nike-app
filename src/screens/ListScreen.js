...
export default class ListScreen extends React.Component {
    ...
    animatedScroll = Animated.cond(
        Animated.eq(this.gestureState, State.BEGAN),
        [
            Animated.debug('began', this.gestureState),
            1
        ],
        [
            Animated.debug('not began', this.gestureState),
            1
        ]
    )
    ...
    _onMomentumScrollEnd = e => {
        this.setState({
            currentScrollPosition: e.nativeEvent.contentOffset.x
        }, () => {
            console.log('state p', this.state.currentScrollPosition)
        })
    }
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                <AddWishButton currentScrollPosition={this.state.currentScrollPosition} listData={this.state.listData}/>
                <ImageBackground
